...
//    @Scheduled(cron = "0 0 0 * * ?")
    public void fetchAndUpdateNotices() {
        log.info("청약홈 공고 데이터 업데이트 스케줄러 시작");

        try {
            List<ApplyHomeDTO> allNotices = fetchNotice(10);
    ...
    public List<ApplyHomeDTO> fetchNotice(int size) {
        ...
        List<ApplyHomeDTO> applyhome_apt = null;
        ...
        try {
            ...
            if (dataNode.isArray()) {
                return mapper.convertValue(
                        dataNode,
                        new TypeReference<List<ApplyHomeDTO>>() {});
